$(document).ready(function () {
	// 기본 Socket
    const socket = io.connect("https://www.janahdaeauctionserver.site", {
    ...
    function initGame() {
        ...
        for (const team_index of Array(5).keys()) {
            ...
            for (const position_index of Array(5).keys()) {
                ...
                {
                    newImage.src = player_small_icon_list[team_index * 5 + 1];
                    $(tmp_str + 'image').html(newImage);
                }
            }

            // 각 팀의 포인트 초기화
            $('.team_info_point').text(get_default_money(team_index));
    ...
    socket.on('enter-game', function (data) {
        ...
        lineup_data = [data.lineup][0];
        time_auction_countdown = data.auction;
        time_wait_countdown = data.wait;
        prepareLineup(lineup_data)
        socket.emit('ready-next');
    });

    function prepareLineup(lineup)
    {
        for(var i = 1; i <= lineup.length; i++)
        {
            var lineupObject = $('#lineup_sequence_' + String(i));
            lineupObject.text(String(i) + ". " + player_list[lineup[i - 1]].name);
            lineupObject.addClass("lineup_td_remain");
        }
    }
    ...
    socket.on('room-id-create-result', function (data) {
        if (data) {
            ...
            $('#create_room_leader').attr('disabled', true);
        } else {
            $('#create_room_id').val("")
    ...
    socket.on('wait-number-change-custom', function (data) {
        setParticipantsInfoText(data);
    });

    function setParticipantsInfoText(data, additional_html = "") {
        var count = data.count;
        var participants = [false, false, false, false, false];
        for(var i = 0; i < data.participants.length; i++)
        {
            participants[data.participants[i]] = true;
        }
        
        var stringConcat = "";
        if(additional_html.length > 0)
            stringConcat = additional_html;
        for(var i = 0; i < manager_list.length; i++)
        {
            console.log(participants, i);
            if(participants[i])
            {
                stringConcat += "<span class='participant_ready'>" + manager_list[i] +" </span>"
            }
            else
            {
                stringConcat += "<span class='participant_empty'>" + manager_list[i] +" </span>"
            }
        }

        stringConcat += " " + count;

        $('#custom_joined_text_create').html(stringConcat);
        $('#custom_joined_text_join').html(stringConcat);
    }
    ...
    function restoreModal() {
        ...
        $('#create_room_leader').attr('disabled', false);
        ...
        $('#join_room_leader').attr('disabled', false);
    ...
    $('#create_room_do').click(function () {
        if (!is_create_room) {
            ...
            var leader = $('#create_room_leader').val();
    ...
    $('#join_room_do').click(function (data) {
        if (!is_join_room) {
            ...
            var leader = $('#join_room_leader').val();
            socket.emit('join-custom', {
                roomId : room_id,
                leader : leader
            });
        } else {
    ...
    socket.on('room-id-participant-exist', function(data) {
        setParticipantsInfoText(data, "<p class='warning_text'>해당 팀장은 선택할 수 없습니다.</p>")
        $('#join_room_id').css("background", color_dict["wrong"]);
    });
    ...
    function SetCurrentPlayerIndex(auction_index, className, isAdd)
    {
        var find_index = lineup_data.findIndex((element) => element == auction_index);
        var lineupObject = $('#lineup_sequence_' + String(find_index + 1));
        if(lineupObject != undefined)
        {
            if(isAdd)
            {
                lineupObject.addClass(className);
            }
            else
            {
                lineupObject.removeClass(className);
            }
        }
    }
    ...
    socket.on('new-lineup', function (data) {
        ...
        SetCurrentPlayerIndex(current_selected, "lineup_td_current", true);
    ...
    socket.on('skip-auction', function () {
        SetCurrentPlayerIndex(current_selected, "lineup_td_current", false);
        SetCurrentPlayerIndex(current_selected, "lineup_td_remain", true);
        skipAuctionInfo();
    });
    ...
    socket.on('get-person', function (data) {
        ...
        SetCurrentPlayerIndex(current_selected, "lineup_td_current", false);
        SetCurrentPlayerIndex(current_selected, "lineup_td_notable", true);
    ...
    $('#action_check').click(function () {
        ...
        var check = validateMoney(newMoney);
        console.log("+5 ", check, newMoney);
        if(check)
        {
            socket.emit('money-change', newMoney);
        }
    });
