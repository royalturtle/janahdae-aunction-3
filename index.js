...
io.on('connection', function(socket){
	...
	// 게임 정보 (방 ID, 대기 시간, 경매 시간)
        ...
        var leader = parseInt(data.leader);
	    
        if(roomId === null || waitTime === null || auctionTime === null || leader === null) { return; }
        ...
        // 방 생성 성공시
	        ...
	        // 방 참가 및 알리기
            lobbyManager.pushCustom(socket, roomId, leader);
    ...
    socket.on('join-custom', function(data) {
        ...
        var isKeyNotExist = lobbyManager.isCustomJoinable(roomId, leader);
        console.log("TEST : ", isKeyNotExist);
        if(isKeyNotExist == 1 || roomId === "")  {
            io.to(socket.id).emit('room-id-join-result', false);
        } 
        else if(isKeyNotExist == 2)
        {
            var currentInfo =  lobbyManager.getCustomStatus(roomId);
            io.to(socket.id).emit('room-id-participant-exist', currentInfo);
        }
        else {
	        // 방 참가 및 알리기
            lobbyManager.pushCustom(socket, roomId, leader);
            ...
            io.to(socket.id).emit('room-id-join-result', true);
        }
    });
